...
public class TopicLayout extends LinearLayout implements View.OnClickListener, ViewPager.OnAdapterChangeListener, ViewPager.OnPageChangeListener {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (!isOnce && getChildCount() > 0) {//当tab全部添加完毕后，会调用此方法
            isOnce = true;//将其置为true,保证此方法只会调用一次
            View child = getChildAt(getChildCount() - 1);//取得最后一个child
            LinearLayout.LayoutParams lp = (LayoutParams) child.getLayoutParams();
            lp.bottomMargin = (int) convertDimension(3);//为了画indicator，所以最后一个child需要设置距离底部的距离
    ...
    private void updateTabs() {
        ...
        /*
         * 判断所有控件是否都已经添加完毕，如果还没有全部添加，则需要再次添加
         */
        if (isAdd) {
            Log.d(TAG, "addView....");
            this.addView(linearLayout);
    ...
    private void updateIndicator(View v, int position) {
        ...
        int x = leftMargin * position;//设置x轴移动的距离
